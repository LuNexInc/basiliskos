...
var state = pluginState{
	config: pluginConfig{MaxConcurrency: 2},
	active:        make(map[string]struct{}),
	compactionReq: make(map[string]bool),
	summary:       make(map[string]*strings.Builder),
...
type pluginState struct {
	...
	compactionReq map[string]bool
	summary       map[string]*strings.Builder
...
func configure(raw []byte) error {
	...
	cfg := pluginConfig{MaxConcurrency: 2}
...
func interceptBeforeAuth(raw []byte) ([]byte, error) {
	...
	// Compaction support for routed models.
	if isCompactionTriggerRequest(req.Body) {
		appendMarker("COMPACTION_DETECTED " + req.RequestID)
		transformed, errRewrite := rewriteCompactionRequestToSummarizer(req.Body)
		if errRewrite != nil {
			return nil, errRewrite
		}
		state.mu.Lock()
		state.compactionReq[req.RequestID] = true
		state.summary[req.RequestID] = &strings.Builder{}
		state.buffer[req.RequestID] = &bytes.Buffer{}
		state.mu.Unlock()
		return okEnvelope(pluginapi.RequestInterceptResponse{Headers: req.Headers, Body: transformed})
	}
	if hasCompactionInputItems(req.Body) {
		appendMarker("REPLAY_DECODE " + req.RequestID)
		transformed, errDecode := decodeCompactionInputItems(req.Body)
		if errDecode != nil {
...
func completeRequest(raw []byte) ([]byte, error) {
	...
	delete(state.compactionReq, completion.RequestID)
	delete(state.summary, completion.RequestID)
	delete(state.buffer, completion.RequestID)
